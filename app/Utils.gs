...
function fetchFromBackend(path) {
  try {
    ...
    console.error("Backend error " + response.getResponseCode() + " for " + path);
  } catch (e) {
...
function extractExcelContent(base64Data, fileName) {
  try {
    ...
    var sheetFile = Drive.Files.copy({ mimeType: "application/vnd.google-apps.spreadsheet" }, tempFile.getId());
...
function processEmailsFiltered(senderFilter, fromDate, toDate) {
  ...
  if (!rulesList) { console.error("No rules found."); return 0; }
  ...
  rulesList.forEach(function(rule) {
    ...
    console.log("Syncing rule [" + rule.id + "] " + rule.name + " | Query: " + query);
    ...
    threads.forEach(function(thread) {
      thread.getMessages().forEach(function(message) {
        ...
        if (result && result.status === "success") {
          ...
          totalCount++;
          console.log("✅ Ingested: " + message.getSubject() + " → rule " + rule.id + " | extracted: " + result.extracted);
        } else {
          console.error("❌ Failed: " + message.getSubject() + " → " + JSON.stringify(result));
        }
      });
    });
  });
...
function triggerFullSync() {
  ...
  return CardService.newActionResponseBuilder()
    .setNotification(CardService.newNotification().setText("✅ Synced " + count + " new emails!"))
    .build();
}
...
function triggerAuthorization() {
  try {
    ...
    return CardService.newActionResponseBuilder()
      .setNotification(CardService.newNotification().setText("⚠️ Check Gmail for authorization banner."))
      .build();
  }
}
...
function debugSync() {
  var rulesList = fetchFromBackend("/rules");
  if (!rulesList) { console.log("No rules."); return; }
  ...
  rulesList.forEach(function(rule) {
    ...
    threads.forEach(function(thread) {
      thread.getMessages().forEach(function(message) {
        ...
        console.log("  " + (processed ? "✅ DONE" : "🔲 NEW") + " | " + message.getSubject());
      });
    });
  });
}