...
function handleFilteredExport(e) {
  ...
  var from = e.formInput.from_date || "";
  var to = e.formInput.to_date || "";

  if (!ruleId) {
    return CardService.newActionResponseBuilder()
      .setNotification(CardService.newNotification().setText("Please enter a Rule ID.")).build();
  }
  ...
  summary.data.forEach(function(item) {
    Object.keys(item.fields).forEach(function(k) { allKeys.add(k); });
  });
  ...
  var rows = summary.data.map(function(item) {
    ...
    keyArr.forEach(function(k) { row.push(item.fields[k] || ""); });
    return row;
  });
...
function handleGenerateSummaryDoc(e) {
  ...
  summary.data.forEach(function(item, idx) {
    ...
    // List attachments
    var atts = Object.keys(fields).filter(function(k) { return k.startsWith("attachment_"); });
    if (atts.length > 0) {
  ...
  return CardService.newActionResponseBuilder()
    .setNotification(CardService.newNotification().setText("📄 Summary Doc created!"))
    .setOpenLink(CardService.newOpenLink().setUrl(doc.getUrl()))
    .build();
}